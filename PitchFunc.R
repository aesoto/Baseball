...
optimal_logit <- function(data, n_cv_splits) {
  #
  # NOTE: I wanted to keep the same API usage, so I kept the `train()`
  #       function call from the `caret` package, but it's not necessary since
  #       Logit models do not have any parameters to optimize for (they simply
  #       maximize the Maximum Likelihood), so the result will be the same
  #       regardless of the cross-validation procedure. Just keep that in
  #       mind.
  #
  set.seed(12345)
  optimal_model <- train(
